...
1. Upload this `.ipynb` source file to [Google Colab](https://colab.research.google.com/).
2. Run the first Code cell (**Khởi tạo & Xác thực tài khoản**) by clicking the **Play (▶)** button.
3. A popup will appear. Sign in with your Google account and grant Colab permission to access your Google Drive. Wait for the `✅ Xác thực thành công!` (Authentication successful) message.
...
1. Run the second Code cell (**Input - Đầu Vào**).
2. Paste the two copied links into their respective input fields.
3. Click the **▶ Khởi chạy Copy** (Start Copy) button.
4. Keep the browser tab open and your computer awake until the tool prints `✅ Done. Đã copy thành công...`.
...
> **👉 Don't worry:** The system will automatically skip these locked files and continue copying the valid ones. The original folder owner will **not** receive any notifications.
...
> **👉 Solution:** If the process is interrupted, the tool **cannot resume** from where it left off. You must go to your Destination Drive, completely delete the partially copied folder, create a brand new empty folder, and run the tool from the beginning.
